...
public extension RiffleSession {
    ...
    // Collection Support
    public func call<A: CollectionType, B: CollectionType, C: CN, D: CN where A.Generator.Element : CN, B.Generator.Element : CN>(pdid: String, _ args: AnyObject..., handler fn: ((A, B, C, D) -> ())?)  {
        _call(pdid, args: args, fn: fn == nil ? nil: cumin(fn!))
    }
...
public func cumin<A: CollectionType, B: CollectionType, C: CN, D: CN where A.Generator.Element : CN, B.Generator.Element : CN>(fn: (A, B, C, D) -> ()) -> ([AnyObject]) -> () {
    return { (a: [AnyObject]) in fn(A.self <- a[0], B.self <- a[1], C.self <- a[2], D.self <- a[3]) }
}
